...
public class UIRatingStar: UIView {
    ...
    private func updateColors() {
        ...
        for index in value..<petals.count {
            ...
            petals[index].strokeColor = getColor(named: "LabelApp")?.cgColor ?? UIColor.yellow.cgColor // Viền màu từ LabelApp
            petals[index].lineWidth = 1.0 // Độ dày viền
        }

        for index in 0..<value {
            petals[index].fillColor = selectionColor.cgColor
            petals[index].strokeColor = UIColor.clear.cgColor // Không viền cho cánh đã chọn
            petals[index].lineWidth = 0.0 // Loại bỏ viền
        }
    ...
    private func getColor(named name: String) -> UIColor? {
        if #available(iOS 11.0, *) {
            ...
            // Màu mặc định nếu không hỗ trợ `UIColor(named:)`
